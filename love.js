(function(window){
    ...
    function inflower(x, y, r, petals) {
        petals = petals || 8;
        ...
        var maxR = r * Math.abs(Math.sin(petals * angle));
    ...
    Flower = function(petals) {
            petals = petals || 6;   // ✅ FIX: ensure petals always exists
            var points = [];
            var size = 18;

            for (var t = 0; t <= Math.PI * 2; t += 0.03) {
                    var r = size * Math.sin(petals * t);
                    var x = r * Math.cos(t);
                    var y = r * Math.sin(t);
                    points.push(new Point(x, y));
            }
    ...
    Tree.prototype = {
        ...
        createBloom: function(width, height, radius, figure, color, alpha, angle, scale, place, speed) {
            ...
            while (true) {
                ...
                if (inflower(x - width / 2, height - (height - 20) / 2 - y, radius)) {
    ...
    Tree.prototype = {
        ...
        createBloom: function(width, height, radius, figure, color, alpha, angle, scale, place, speed) {
            ...
            while (true) {
                x = random(20, width - 20);
                y = random(20, height - 20);
                if (inflower(x - width / 2, height - (height - 60) / 2 - y, radius)) {
                    return new Bloom(this, new Point(x, y), figure, color, alpha, angle, scale, place, speed);
                }
            }
        },
